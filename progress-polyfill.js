...
(function() {
  ...
  (function($) {
    ...
    $.fn.html5Progress = function() {
      var updatePolyfill;
      updatePolyfill = function(progressElem, progressBarDiv) {
        var amt, max, val, _ref;
        _ref = $([$(progressElem).attr('max') || '1.0', $(progressElem).attr('value')]).map(function() {
          if (/^\-?\d+(?:\.\d+)?$/.test(this)) {
            return parseFloat(this);
          }
        }).get(), max = _ref[0], val = _ref[1];
        if (val > max) {
          val = max;
        }
        amt = max > 0 ? (val / max) * 100.0 : 0;
        progressBarDiv.style.width = amt + "%";
        return null;
      };
      $(this).filter('progress[value]').each(function() {
        var $elem, attrHandler, attrMutationCallback, attrObserver, progressBarDiv, progressFrameDiv,
          _this = this;
        $elem = $(this);
        ...
        if ((typeof WebKitMutationObserver !== "undefined" && WebKitMutationObserver !== null) || (typeof MutationObserver !== "undefined" && MutationObserver !== null)) {
          attrMutationCallback = function(mutations, observer) {
            ...
            for (_i = 0, _len = mutations.length; _i < _len; _i++) {
              ...
              if (mutation.type === "attributes" && ((_ref = mutation.attributeName) === "value" || _ref === "max")) {
                if ($elem.is("[value]")) {
                  updatePolyfill(_this, progressBarDiv);
                } else {
          ...
          attrHandler = function(evt) {
            var _ref;
            if ((_ref = evt.originalEvent.attrName) === "value" || _ref === "max") {
              if ($elem.is("[value]")) {
                updatePolyfill(_this, progressBarDiv);
              } else {
                $elem.children("div.progress-frame").remove();
      ...
      $(this).filter('progress:not([value])').each(function() {
        var $elem, attrHandler, attrMutationCallback, attrObserver, progressDiv,
          _this = this;
        $elem = $(this);
